...
const PuzzlePiece = ({ gameType, title, isCompleted, pieceNumber, onClick }: PuzzlePieceProps) => {
  const getIcon = () => {
    switch (gameType) {
      case 'top10': return <Star className="w-8 h-8" />;
      case 'predict_future': return <Lightbulb className="w-8 h-8" />;
      case 'drawful': return <Puzzle className="w-8 h-8" />;
      case 'would_you_do': return <Heart className="w-8 h-8" />;
    }
  };
  ...
  return (
    <Card className={`relative overflow-hidden transition-all duration-300 hover:scale-105 ${getPuzzleShape()}`}>
      <Button
        onClick={onClick}
