  ...
  const createOrJoinSession = async () => {
    ...
    try {
      ...
      if (existingSessions && existingSessions.length > 0) {
        ...
        const updateField = userSelection === 'Guille' ? 'guille_connected' : 'delfina_connected';
      ...
      setIsConnected(true);
      setBothConnected(currentSession.guille_connected && currentSession.delfina_connected);
    } catch (error) {
    ...
    const sessionChannel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          setBothConnected(updatedSession.guille_connected && updatedSession.delfina_connected);
        }
      )
      .subscribe();
